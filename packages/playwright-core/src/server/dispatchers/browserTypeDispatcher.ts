...
export class BrowserTypeDispatcher extends Dispatcher<BrowserType, channels.BrowserTypeChannel, RootDispatcher> implements channels.BrowserTypeChannel {
  ...
  async launchPersistent(params: channels.BrowserTypeLaunchPersistentParams, metadata: CallMetadata): Promise<channels.BrowserTypeLaunchPersistentResult> {
    const browser = await this._object.launchPersistent(metadata, params.userDataDir, params);
    const browserDispatcher = new BrowserDispatcher(this, browser);
    return { browser: browserDispatcher, defaultContext: browser._defaultContext ? new BrowserContextDispatcher(browserDispatcher, browser._defaultContext) : undefined };
  }
